...
public class MobileNetworkUtils {
    ...
    public static boolean isShow4gOnly(Context context, int subId) {
        if (SubscriptionManager.isValidSubscriptionId(subId)) {
            CarrierConfigManager carrierMgr = (CarrierConfigManager) context
                    .getSystemService(Context.CARRIER_CONFIG_SERVICE);
            if ((carrierMgr != null)
                    && (carrierMgr.getConfigForSubId(subId) != null)) {
                return carrierMgr.getConfigForSubId(subId).getBoolean(
                        CarrierConfigManagerEx.KEY_OEM_PREFER_4G_ONLY, false);
            }
        }
        return false;
    }

    public static boolean isShow3gOnly(Context context, int subId) {
        if (SubscriptionManager.isValidSubscriptionId(subId)) {
            CarrierConfigManager carrierMgr = (CarrierConfigManager) context
                    .getSystemService(Context.CARRIER_CONFIG_SERVICE);
            if ((carrierMgr != null)
                    && (carrierMgr.getConfigForSubId(subId) != null)) {
                return carrierMgr.getConfigForSubId(subId).getBoolean(
                        CarrierConfigManagerEx.KEY_OEM_PREFER_3G_ONLY, false);
            }
        }
        return false;
    }
