...
public class WifiDetailPreferenceController extends AbstractPreferenceController
    ...
    private ActionButtonsPreference mDisconnectButtonPref;
    static final String KEY_BUTTONS_DISCONNECT_PREF = "disconnect_buttons";
                ...
                case WifiManager.CONFIGURED_NETWORKS_CHANGED_ACTION:
                    if (!intent.getBooleanExtra(WifiManager.EXTRA_MULTIPLE_NETWORKS_CHANGED,
                        ...
                        if (mAccessPoint.matches(wifiConfiguration)) {
                            ...
                            // When AccessPoint config change, keep WifiPrivacyPreferenceController get latest config
                            // because WifiPrivacyPreferenceController has handled macRandomizationSetting changed, so only other config change, need update
                            if (mWifiPrivacyPreferenceController != null &&
                                mWifiConfig.macRandomizationSetting == wifiConfiguration.macRandomizationSetting) {
                                mWifiPrivacyPreferenceController.setWifiConfiguration(mWifiConfig);
                            }
                            // When AccessPoint config change, keep WifiMeteredPreferenceController get latest config
                            // because WifiMeteredPreferenceController has handled meteredOverride changed, so only other config change, need update
                            if (mWifiMeteredPreferenceController != null &&
                                mWifiConfig.meteredOverride == wifiConfiguration.meteredOverride) {
                                mWifiMeteredPreferenceController.setWifiConfiguration(mWifiConfig);
                            }
                        }
                    }
                    // fall through
    ...
    private final Runnable failedStateRunnable = new Runnable() {
        @Override
        public void run() {
    ...
    private WifiPrivacyPreferenceController mWifiPrivacyPreferenceController;
    private WifiMeteredPreferenceController mWifiMeteredPreferenceController;
    ...
    @VisibleForTesting
        ...
        mConnectingState = mConnected ? STATE_CONNECTED : STATE_NONE;
        mConnectListener = new WifiManager.ActionListener() {
            ...
            public void onFailure(int reason) {
                mHandler.post(failedStateRunnable);
            }
        };
    }

    public void setMeteredPreferenceController(WifiMeteredPreferenceController meteredPreferenceController) {
        mWifiMeteredPreferenceController = meteredPreferenceController;
    }

    public void setPrivacyPreferenceController(WifiPrivacyPreferenceController privacyPreferenceController) {
        mWifiPrivacyPreferenceController = privacyPreferenceController;
    }
    ...
    public void displayPreference(PreferenceScreen screen) {
        ...
        mDisconnectButtonPref = (ActionButtonsPreference) screen.findPreference(KEY_BUTTONS_DISCONNECT_PREF);
        if (WifiUtils.isDisableNetworkSupported(mContext)) {
            mDisconnectButtonPref.setButton1Text(R.string.wifi_disconnect_button_text)
                                 .setButton1Icon(R.drawable.ic_settings_disable)
                                 .setButton1OnClickListener(view -> disableNetwork());
        } else {
            mDisconnectButtonPref.setButton1Visible(false);
        }

        if (WifiUtils.isDisconnectNetworkSupported(mContext)) {
            mDisconnectButtonPref.setButton2Text(R.string.wifi_disconnect_button_text)
                                 .setButton2OnClickListener(view -> disconnectNetwork());
        } else {
            mDisconnectButtonPref.setButton2Visible(false);
        }

        if (WifiUtils.isDisableNetworkSupported(mContext) || WifiUtils.isDisconnectNetworkSupported(mContext)) {
            if (mAccessPoint != null && mAccessPoint.isActive()) {
                mDisconnectButtonPref.setButton3Visible(false);
                mDisconnectButtonPref.setButton4Visible(false);
            } else {
                mDisconnectButtonPref.setVisible(false);
            }
        } else {
            mDisconnectButtonPref.setVisible(false);
        }
    ...
    private boolean canForgetNetwork() {
        if (mWifiConfig != null && !mWifiConfig.canForget) {
            return false;
        }
    ...
    public boolean canModifyNetwork() {
        return mWifiConfig != null && mWifiConfig.canModify && !WifiUtils.isNetworkLockedDown(mContext, mWifiConfig);
    }
